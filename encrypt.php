...
EOF;
		...
		$next_file = true;
		$tmp;
		foreach(file("../../secret/uncompress-epub/OPS/package.opf") as $line){
		...
		$write_file = "uncompress-epub/OPS/".$list_of_content[$current_file];
		$resfile = fopen($write_file, "w");
		//smart quotes don't get encrypted nicely :(
		//$search = array(chr(145), chr(146), chr(147), chr(148), chr(151)); 
		//$replace = array("'", "'", '"', '"', '-'); 
		//123 * 16 = 1968bits = max to encode with a 2048 key
		foreach(str_split(file_get_contents("../../secret/uncompress-epub/OPS/".$list_of_content[$current_file]),123) as $val){
